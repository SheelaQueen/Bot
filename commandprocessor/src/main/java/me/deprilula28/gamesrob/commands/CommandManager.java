...
public class CommandManager {
    ...
    public static String help(CommandContext context) {
        ...
        if (System.currentTimeMillis() - Statistics.get().getLastUpdateLogSentTime() <= TimeUnit.DAYS.toMillis(1))
            embed.appendDescription(Language.transl(Utility.getLanguage(context), "command.help.recentUpdate",
                    GamesROB.VERSION, Utility.getPrefix(context.getGuild()))).appendDescription("\n");
        ...
        Consumer<MessageBuilder> messageBuilder = builder -> builder.append(languageHelpMessages.get(Utility.getLanguage(context))
                .replaceAll("%PREFIX%", Utility.getPrefixHelp(context.getGuild()))).setEmbed(
                embed.build());
