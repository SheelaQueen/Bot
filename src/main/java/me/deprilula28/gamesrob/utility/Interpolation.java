...
public abstract class Interpolation {
    ...
    static public class Elastic extends Interpolation {
        ...
        public float apply (float a) {
            if (a <= 0.5f) {
                a *= 2;
                return (float)Math.pow(value, power * (a - 1)) * (float) Math.sin(a * bounces) * scale / 2;
            }
            a = 1 - a;
            a *= 2;
            return 1 - (float)Math.pow(value, power * (a - 1)) * (float) Math.sin((a) * bounces) * scale / 2;
        }
    }
    ...
    static public class ElasticOut extends Elastic {
        ...
        public float apply (float a) {
            if (a == 0) return 0;
            a = 1 - a;
            return (float) (1 - (float)Math.pow(value, power * (a - 1)) * Math.sin(a * bounces) * scale);
        }
    }
